class DriversController < ApplicationController
  ...
  def register
    ...
    return render json: {status:"ERROR",message: "As the driver, you have already taken place"} if dr and dr.driver
  ...
  def order
    ...
    gps_lat_drivers = params[:gps_lat_drivers].to_f
    ...
    return render :json => result.map{|result|{status:"OK", :id_result=> result.id, :user_id=> result.user_id, :dist=>result.distance.to_f, :address=> result.address }} if result!=nil                                                                                                                                                #* 6378245
  ...
  def took
    ...
    o.update_attribute(:status, Order::Status::CLOSED) if o.status==Order::Status::UsersVillages
    o.update_attribute(:status, Order::Status::DriversTook) if o.status== Order::Status::SELECT
